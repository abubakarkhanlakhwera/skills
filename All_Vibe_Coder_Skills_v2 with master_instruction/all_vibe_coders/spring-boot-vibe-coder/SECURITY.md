# 🔒 SECURITY.md — Spring Boot Security Checklist
...
- Use Spring Security's method security (@PreAuthorize) for resource-level checks
- Hash passwords with BCryptPasswordEncoder — never plain text or MD5
- Set HSTS, X-Frame-Options, CSP headers via Spring Security's headers configuration
- Use OAuth2 Resource Server for JWT validation — never manual JWT parsing
- Validate all request DTOs with @Validated + Bean Validation annotations
