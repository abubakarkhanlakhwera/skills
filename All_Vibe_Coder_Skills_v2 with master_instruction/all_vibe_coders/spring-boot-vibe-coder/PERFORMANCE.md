# ⚡ PERFORMANCE.md — Spring Boot Performance Rules
...
## Spring Boot-Specific Performance Rules
- Use Spring Data JPA projections (interfaces) to select only needed columns
- Use @EntityGraph to avoid N+1 on associations
- Use Spring Cache (@Cacheable) for expensive read operations
- Use async (@Async) for fire-and-forget work
- Use Flyway with index creation scripts for all FK columns
