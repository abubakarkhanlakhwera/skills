# 🔒 SECURITY.md — Django REST Security Checklist
> Stack-specific security rules for this project.
> The AI must check these before suggesting any auth, data, or config code.

## Django REST-Specific Security Rules
- Use Django's built-in CSRF protection — never exempt without justification
- Always use parameterized ORM queries — never .raw() with user input
- Set ALLOWED_HOSTS, SECURE_SSL_REDIRECT, SESSION_COOKIE_SECURE in production
- Use django-cors-headers with explicit CORS_ALLOWED_ORIGINS — never CORS_ALLOW_ALL
- Rotate SECRET_KEY per environment — never commit to VCS
