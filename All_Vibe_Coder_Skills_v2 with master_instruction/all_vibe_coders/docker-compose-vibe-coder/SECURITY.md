# 🔒 SECURITY.md — Docker + Compose Security Checklist
> Stack-specific security rules for this project.
> The AI must check these before suggesting any auth, data, or config code.

## Docker + Compose-Specific Security Rules
- Never commit .env files — .env.example with dummy values only
- Use Docker secrets for sensitive values in Swarm — not environment variables
- Set read_only: true on containers that don't need to write
- Use USER appuser in every production Dockerfile
- Limit container capabilities with cap_drop: [ALL] and add only what's needed
