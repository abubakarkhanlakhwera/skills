# 🔒 SECURITY.md — Streamlit Security Checklist
> Stack-specific security rules for this project.
> The AI must check these before suggesting any auth, data, or config code.

## Streamlit-Specific Security Rules
- Secrets in .streamlit/secrets.toml — never in source code
- Use streamlit-authenticator for user authentication
- Validate all user inputs from st.text_input, st.file_uploader before processing
- Use st.secrets for API keys — never os.environ in deployed apps
