# 🔒 SECURITY.md — Vue + Nuxt Security Checklist
> Stack-specific security rules for this project.
> The AI must check these before suggesting any auth, data, or config code.

## Vue + Nuxt-Specific Security Rules
- Validate all server route inputs with Zod
- Auth checks in server/middleware/auth.ts applied to protected routes
- Use useRuntimeConfig() for env — never process.env in components
- Set security headers via nuxt-security module
- Never expose private env vars to client-side config
