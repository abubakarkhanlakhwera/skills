# ⚡ PERFORMANCE.md — FastAPI Performance Rules
...
## FastAPI-Specific Performance Rules
- Use async def for all I/O-bound routes — never sync DB calls in async context
- Use select() with specific columns — never SELECT *
- Add DB indexes for all foreign keys and frequently filtered columns
- Use background tasks (BackgroundTasks) for fire-and-forget work
- Cache expensive computations with Redis via aiocache
