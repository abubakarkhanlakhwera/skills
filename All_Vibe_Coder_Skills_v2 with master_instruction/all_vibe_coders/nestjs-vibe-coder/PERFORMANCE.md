# ⚡ PERFORMANCE.md — NestJS Performance Rules
...
## NestJS-Specific Performance Rules
- Use caching interceptor (@UseInterceptors(CacheInterceptor)) for read-heavy endpoints
- Use Prisma select or TypeORM select: {} to fetch only needed columns
- Use Bull queue for async jobs — never block request cycle for slow work
- Use NestJS compression middleware for API responses
